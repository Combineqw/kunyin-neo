...
/** 唯一允许的来源：系统音乐文件夹。不做任何全盘搜索。 */
const MUSIC_ROOT = join(homedir(), 'Music')
...
rule()
line()
line('  从系统「音乐」文件夹复制真实歌曲到测试素材目录。')
line('  只读音乐文件夹、只写测试目录，不做全盘扫描。')
line()
line('  音乐文件夹：' + MUSIC_ROOT)
line('  测试目录：  ' + outDir)
line('  目标数量：  ' + TARGET + ' 首')
line()

if (!existsSync(MUSIC_ROOT)) {
  line('  ✗ 找不到系统音乐文件夹：' + MUSIC_ROOT)
  line('    请确认该目录存在后重试。')
  line()
  process.exit(1)
}
...
if (manifest.meetsTarget) {
  ...
  line('  请往下面这个文件夹再放 ' + gap + ' 首歌，然后重新运行本脚本：')
  line('      ' + MUSIC_ROOT)
}
rule('-')
line()

...
export const TABLE_PLAY_EVENTS = 'play_events'
export const TABLE_SONG_METADATA = 'song_metadata'
export const TABLE_RECOMMENDATION_CACHE = 'recommendation_cache'
...
/** 当前 schema 版本（v6=Android 终态；v7=sort_order；v8=播放事件/元数据；v9=每日推荐缓存）。 */
...
function createSchema(d: Database.Database): void {
  d.exec(`
    ...
    CREATE TABLE IF NOT EXISTS ${TABLE_PLAY_EVENTS} (
    ...
    CREATE INDEX IF NOT EXISTS idx_play_events_song ON ${TABLE_PLAY_EVENTS}(song_id, source);
    CREATE INDEX IF NOT EXISTS idx_play_events_time ON ${TABLE_PLAY_EVENTS}(played_at);

    CREATE TABLE IF NOT EXISTS ${TABLE_SONG_METADATA} (
      song_id INTEGER NOT NULL,
      source TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_song_metadata_genre ON ${TABLE_SONG_METADATA}(genre);

    CREATE TABLE IF NOT EXISTS ${TABLE_RECOMMENDATION_CACHE} (
...
export function getDb(): Database.Database {
  ...
  if (current < SCHEMA_VERSION) {
    // 新库由 createSchema 建成终态；旧库仅需补齐 v7 的排序列，v8 新表由 createSchema 创建。

...
const trayIconIco = join(iconDirectory, 'icon.ico')
const trayIconPng = join(iconDirectory, '32x32.png')
const thumbPrev = join(iconDirectory, 'thumb-prev.png')
const thumbPlay = join(iconDirectory, 'thumb-play.png')
const thumbPause = join(iconDirectory, 'thumb-pause.png')
const thumbNext = join(iconDirectory, 'thumb-next.png')
...
export function registerMediaModule(): void {
  ...
  // 系统托盘属于增强能力；某些无托盘的桌面会话中 native Tray 构造可能阻塞，
  // 不能阻断主窗口创建。延后到当前启动批次完成后再初始化，失败时保留主窗口和媒体键。
  setImmediate(() => {
    try {
      buildTray()
    } catch (error) {
      console.warn('[media] 系统托盘初始化失败，已降级为无托盘模式', error)
    }
  })
  applyAutoLaunch()

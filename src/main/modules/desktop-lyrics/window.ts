...
function createLyricWindow(): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    useContentSize: true,
    frame: false,
    // Linux 缺少合成器时透明窗口不可靠，维持功能但使用不透明深色底降级。
    transparent: process.platform !== 'linux',
    backgroundColor: process.platform === 'linux' ? '#171918' : '#00000000',
  ...
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    ...
    void win.loadURL(`${process.env['ELECTRON_RENDERER_URL']}/desktop-lyrics.html`).catch((error) => {
      console.error('[desktop-lyric] loadURL rejected', error)
    })
  } else {
    // 修复: 打包后的歌词入口加载失败也必须被消费，防止主进程静默拒绝。
    void win.loadFile(join(__dirname, '../renderer/desktop-lyrics.html')).catch((error) => {
      console.error('[desktop-lyric] loadFile rejected', error)
    })
  }
 ...
 * 创建或显示桌面歌词悬浮窗。
 *
 * @returns 无返回值；已有窗口会被复用以保留最后歌词状态。
 */

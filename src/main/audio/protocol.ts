...
/**
 * 注册给 `kunyin://` 的单条音频流描述。
 * 远程 URL 和本地文件路径互斥；可选 ekey 指示主进程按 Range 偏移流式解密。
 */
...
async function fetchUpstreamOnce(
  ...
  // 修复: AbortSignal 已取消时不会补发 abort 事件；先同步中止才能避免换歌后的孤儿上游请求。
  if (rendererSignal.aborted) abort.abort()
  else rendererSignal.addEventListener('abort', () => abort.abort(), { once: true })
  let timedOut = false

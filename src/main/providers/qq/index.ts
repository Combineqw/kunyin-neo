...
export class QqProvider extends BaseProvider {
  ...
  async searchPlaylist(keyword: string, page = 0, size = 20): Promise<PlaylistSearchResult> {
    const req = {
      comm: this.authComm({
        _channelid: '0',
        _os_version: '6.2.9200-2',
      ...
      'music.search.SearchCgiService': {
        ...
        param: {
          ...
          page_num: page + 1,
          query: keyword,
          remoteplace: 'sizer.newclient.songlist',
          search_type: 3,
          searchid: pcSearchId()
        }
      }
    ...
    if (!data) return this.emptyPage(page, size)
    const total = num(data.meta?.sum, 0)
    const result = (data.body?.songlist?.list ?? data.body?.songlist ?? [])
      .map((pl: any) => this.parsePlaylistEntry(pl))
      .filter((x: any): x is PlayListInfoResult => !!x)
    return { source: 'qq', hasNext: (page + 1) * size < total, page, size, result }
  }
  ...
  private parsePlaylistEntry(o: any): PlayListInfoResult | null {
    const id = o.dissid ?? o.tid ?? o.dirid ?? o.id
    const name = stripEm(o.dissname ?? o.title ?? o.dirname ?? o.dirName ?? '')
    if (id == null || !name) return null
    const creator = typeof o.creator === 'object' ? o.creator?.name : o.creator
    return {
      source: 'qq',
      id: String(id),
      name,
      cover: o.logo ?? o.picurl ?? o.pic_url ?? o.cover,
      creator: creator ?? o.nickname ?? o.host_nick,
      description: o.desc ?? o.description,
      playCount: num(o.listennum ?? o.playcnt ?? o.playCount, 0),
      total: num(o.songnum ?? o.songNum ?? o.song_count, 0)
    }
  }

...
export class WyProvider extends BaseProvider {
  ...
  async searchPlaylist(keyword: string, page = 0, size = 20): Promise<PlaylistSearchResult> {
    const json = await eapiPost('/api/v1/search/playlist/get', {
      s: keyword,
      limit: String(size),
      offset: String(page * size),
      type: 1000,
    ...
    if (!json || json.code !== 200) return this.emptyPage(page, size)
    const total = num(json.result?.playlistCount, 0)
    const result = (json.result?.playlists ?? [])
  ...
  private parsePlaylistEntry(o: any): PlayListInfoResult | null {
    ...
    return {
      ...
      id: String(o.id),
      name: o.name,
      cover: o.coverImgUrl ?? o.coverImgUrlStr ?? o.picUrl,
      creator: o.creator?.nickname ?? o.creatorName ?? o.userName,
      description: o.description,
      playCount: num(o.playCount ?? o.playCountText, 0),
      total: num(o.trackCount ?? o.trackNumberUpdate ?? o.songCount, 0)
    }
  }

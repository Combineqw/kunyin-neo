...
export function registerSettingsHandlers(): void {
  ...
  const commitSettings = (patch: DeepPartial<AppSettings>): AppSettings => {
  ...
  handle(IpcChannels.SETTINGS_SET, commitSettings)
  ipcMain.on(IpcChannels.SETTINGS_SET_SYNC, (event, patch: DeepPartial<AppSettings>) => {
    event.returnValue = commitSettings(patch)
  })

...
import type { IrsProfileImportResult } from './irs'
import type {
  PluginActionResult,
  PluginRepoStatus,
  PluginStoreEvent,
...
export const IpcChannels = {
  ...
  SETTINGS_SET_SYNC: 'settings:set-sync',
  ...
  // 播放行为与本地推荐候选
  PLAYLOG_RECORD: 'playlog:record',
  RECOMMENDATION_SONGS: 'recommendation:songs',
  RECOMMENDATION_DAILY: 'recommendation:daily',
  ...
  MINI_PLAYER_PUSH: 'miniPlayer:push', // 主窗口 → 主：推送播放状态
  MINI_PLAYER_STATE: 'miniPlayer:state', // 主 → 迷你窗口：转发状态
  MINI_PLAYER_COMMAND: 'miniPlayer:command', // 迷你窗口 → 主 → 主窗口：控制播放
  MINI_PLAYER_SET_EXPANDED: 'miniPlayer:setExpanded', // 迷你窗口 → 主：悬停展开/收缩
  ...
  DIALOG_SELECT_OPEN_FILE: 'dialog:selectOpenFile',
  DIALOG_SELECT_SAVE_FILE: 'dialog:selectSaveFile',
  EQ_IMPORT_FILE: 'equalizer:importFile',
  EQ_EXPORT_FILE: 'equalizer:exportFile',
  ANIMATION_IMPORT_FILE: 'animation:importFile',
  ANIMATION_EXPORT_FILE: 'animation:exportFile',
  ...
  // 插件商店
  PLUGIN_STORE_LIST: 'pluginStore:list',
  /** 选择/拖入本地 zip 插件包并解析暂存 */
  PLUGIN_STORE_STAGE_LOCAL: 'pluginStore:stageLocal',
  PLUGIN_STORE_PICK_LOCAL: 'pluginStore:pickLocal',
  PLUGIN_STORE_INSTALL: 'pluginStore:install',
  PLUGIN_STORE_UNINSTALL: 'pluginStore:uninstall',
  PLUGIN_STORE_SET_ENABLED: 'pluginStore:setEnabled',
  PLUGIN_STORE_CHANGED: 'pluginStore:changed', // 主 → 渲染 事件
  // 自定义插件源（地址由用户运行时输入）
  PLUGIN_REPO_LIST: 'pluginRepo:list',
  PLUGIN_REPO_ADD: 'pluginRepo:add',
  PLUGIN_REPO_REMOVE: 'pluginRepo:remove',
  PLUGIN_REPO_SET_ENABLED: 'pluginRepo:setEnabled'
} as const
...
/** 迷你播放器控制命令（迷你窗口回投主窗口播放器） */
export type MiniPlayerCommand = 'playpause' | 'next' | 'prev'
...
export interface WindowApi {
  ...
  settings: {
    ...
    /** 退出前同步落盘；仅供低频最终快照使用，禁止放在高频 UI 事件中 */
    setSync(patch: DeepPartial<AppSettings>): AppSettings
  ...
  playlog: {
    /** 异步落库，不应阻塞播放器切歌。 */
    record(event: PlayEventPayload): Promise<void>
  }
  ...
  miniPlayer: {
    ...
    toggle(enabled: boolean): Promise<void>
    /** 主窗口推送当前曲目与进度 */
    push(state: MiniPlayerState): void
    /** 悬浮窗订阅状态，返回取消订阅 */
    onState(cb: (state: MiniPlayerState) => void): Unsubscribe
    /** 悬浮窗将播放控制回投主窗口 */
  ...
  shell: {
    ...
    /** 弹文件打开框，返回所选文件路径；取消返回 null */
    selectOpenFile(filters: { name: string; extensions: string[] }[]): Promise<string | null>
    /** 弹文件保存框，返回目标文件路径；取消返回 null */
    selectSaveFile(
      defaultName: string,
  ...
  /** EQ 调音文件 */
  equalizer: {
    importProfile(): Promise<EqualizerProfileImportResult | null>
    exportProfile(profile: EqualizerProfile): Promise<string | null>
  }

  /** 动效包 JSON 文件 */
  animation: {
    /** 导入动效包 JSON，取消时返回 null */
    importPack(): Promise<AnimationPackJson | null>
    /** 导出动效包 JSON，返回文件名；取消时返回 null */
    exportPack(pack: AnimationPackJson): Promise<string | null>
  }

  /** 主题 JSON 文件 */
  theme: {
    /** 导入主题文件；取消时返回 null，格式非法时 reject（消息可直接展示） */
  ...
  /** IRS 脉冲响应文件 */
  irs: {
    importProfile(): Promise<IrsProfileImportResult | null>
  }
  ...
  /** 插件商店；安装与卸载均由主进程执行，renderer 只消费状态。 */
  pluginStore: {
    list(): Promise<PluginStoreResult>
    install(id: string): Promise<PluginActionResult>
    uninstall(id: string): Promise<PluginActionResult>
    setEnabled(id: string, enabled: boolean): Promise<PluginActionResult>
    /** 解析并暂存一个本地 zip 插件包（拖入场景传入文件路径） */
    stageLocal(filePath: string): Promise<PluginActionResult>
    /** 弹文件选择器挑一个 zip 并暂存；取消返回 null */
    pickLocal(): Promise<PluginActionResult | null>
    onChange(cb: (event: PluginStoreEvent) => void): Unsubscribe
  }

  /** 自定义插件源。地址全部由用户运行时输入，代码中无预置地址。 */
  pluginRepo: {
    list(): Promise<PluginRepoStatus[]>
    /** 添加并立即试拉一次；地址非法或索引格式不对会抛错 */
    add(url: string, name: string): Promise<PluginRepoStatus>
    remove(id: string): Promise<void>
    setEnabled(id: string, enabled: boolean): Promise<PluginRepoStatus>
  }

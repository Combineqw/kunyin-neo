...
/** 随「暗色主题」勾选切换的三个色值。
 *
 * save() 会把表单里所有色值都写进配置，所以这三项若停在浅色默认值，
 * 勾了暗色也会得到：纯白内容区 + 浅绿侧栏 + 近黑字（对比 16:1 的「深色」主题）。
 * 深色一侧取值与内置深色主题一致（面见 themes.ts 的 DARK_SURFACE，
 * 字色 221 是压掉峰值光渗后的值），侧栏透明则让背景图从窄带露出。 */
type ModeColorKey = 'font' | 'appBackground' | 'contentBackground'
const MODE_COLORS: Record<'light' | 'dark', Record<ModeColorKey, string>> = {
  light: { font: '#212121', appBackground: '#edf7f2', contentBackground: '#ffffff' },
  dark: { font: '#dddddd', appBackground: '#000000', contentBackground: '#252527' }
}
...
/** 勾选/取消「暗色主题」时把字色与两个面切到对应模式的默认值。
 *
 * 只改「还停在另一模式默认值」的那几项——用户自己调过的颜色一律不动，
 * 否则会把人刚选好的色覆盖掉。 */
watch(
  () => form.isDark,
  (isDark) => {
    const from = isDark ? MODE_COLORS.light : MODE_COLORS.dark
    const to = isDark ? MODE_COLORS.dark : MODE_COLORS.light
    for (const key of Object.keys(to) as ModeColorKey[]) {
      if (form[key].toLowerCase() === from[key]) form[key] = to[key]
    }
  }
)
...
/** 字色与内容区底色的 WCAG 对比度。低于 4.5 正文就不达标，高于 12.5（深色下）
 * 会光渗刺眼——手挑颜色很容易两头都踩到，所以在保存前把数值摆出来。 */
const contrast = computed(() => {
  ...
  const [hi, lo] = [lum(form.font), lum(form.contentBackground)].sort((a, b) => b - a)
  const ratio = (hi + 0.05) / (lo + 0.05)
  return {
    ratio,
    text: `字色 / 内容区对比 ${ratio.toFixed(1)}:1`,
    hint:
      ratio < 4.5
        ? '偏低，正文会看不清'
        : form.isDark && ratio > 12.5
          ? '偏高，深色下容易光渗刺眼'
          : '合适',
    bad: ratio < 4.5 || (form.isDark && ratio > 12.5)
  }
})
...
<template>
  <div v-if="modelValue" class="mask" @click.self="emit('update:modelValue', false)">
    <section class="dialog" :class="{ previewing: form.preview }" :style="dialogStyle">
      ...
      <p class="contrast" :class="{ bad: contrast.bad }">

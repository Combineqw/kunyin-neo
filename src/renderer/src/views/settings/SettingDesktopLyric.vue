...
const previewStyle = computed<Record<string, string>>(() => {
  ...
  const background = `rgba(9, 12, 11, ${Math.max(0, Math.min(0.8, s.desktopBgOpacity))})`
  return {
    background: s.desktopGradientBar
      ? `linear-gradient(90deg, transparent, ${background}  ${Math.max(0, (100 - s.desktopGradientWidth) / 2)}%, ${background} ${Math.min(100, 100 - (100 - s.desktopGradientWidth) / 2)}%, transparent)`
      : background,
    fontFamily: s.desktopFont ? `"${s.desktopFont}"` : 'inherit',
    fontSize: `${Math.max(16, Math.min(34, s.desktopFontSize * 0.72))}px`,
    fontWeight: String(s.desktopFontWeight),
    textAlign: s.desktopAlign,
    textShadow: s.desktopShadowEnabled
      ? `${s.desktopShadowX}px ${s.desktopShadowY}px ${s.desktopShadowBlur}px rgba(0, 0, 0, ${s.desktopShadowOpacity})`
      : 'none',
    WebkitTextStroke: `${s.desktopStrokeWidth}px ${s.desktopStrokeColor}`,
...
function setColor(
  key: 'desktopColorNormal' | 'desktopColorActive' | 'desktopStrokeColor',
  event: Event
): void {
...
function resetColors(): void {
  ...
  updateLyrics({
    ...
    desktopStrokeColor: defaults.desktopStrokeColor
  })
}
...
<template>
  ...
  <dd>
    ...
    <div class="range-grid">
      ...
      <label class="range-item">
        <span
          >基础字重 <b>{{ settings.lyrics.desktopFontWeight }}</b></span
        >
        <input
          type="range"
          min="100"
          max="900"
          step="100"
          :value="settings.lyrics.desktopFontWeight"
          @change="setNumber('desktopFontWeight', $event, 100, 900)"
        />
      </label>
      <label class="range-item">
        <span
      ...
      <label class="range-item">
        <span
          >阴影水平偏移 <b>{{ settings.lyrics.desktopShadowX }} px</b></span
        >
        <input
          type="range"
          min="-12"
          max="12"
          step="1"
          :value="settings.lyrics.desktopShadowX"
          @change="setNumber('desktopShadowX', $event, -12, 12)"
        />
      </label>
      <label class="range-item">
        <span
          >阴影垂直偏移 <b>{{ settings.lyrics.desktopShadowY }} px</b></span
        >
        <input
          type="range"
          min="-12"
          max="12"
          step="1"
          :value="settings.lyrics.desktopShadowY"
          @change="setNumber('desktopShadowY', $event, -12, 12)"
        />
      </label>
      <label class="range-item">
        <span
          >阴影模糊 <b>{{ settings.lyrics.desktopShadowBlur }} px</b></span
        >
        <input
          type="range"
          min="0"
          max="24"
          step="1"
          :value="settings.lyrics.desktopShadowBlur"
          @change="setNumber('desktopShadowBlur', $event, 0, 24)"
        />
      </label>
      <label class="range-item">
        <span
          >阴影不透明度 <b>{{ Math.round(settings.lyrics.desktopShadowOpacity * 100) }}%</b></span
        >
        <input
          type="range"
          min="0"
          max="100"
          step="2"
          :value="settings.lyrics.desktopShadowOpacity * 100"
          @change="setNumber('desktopShadowOpacity', $event, 0, 1, 100)"
        />
      </label>
      <label class="range-item">
        <span
          >渐变底条宽度 <b>{{ settings.lyrics.desktopGradientWidth }}%</b></span
        >
        <input
          type="range"
          min="0"
          max="100"
          step="2"
          :value="settings.lyrics.desktopGradientWidth"
          @change="setNumber('desktopGradientWidth', $event, 0, 100)"
        />
      </label>
      <label class="range-item">
        <span
          >渐变底条浓度 <b>{{ Math.round(settings.lyrics.desktopGradientOpacity * 100) }}%</b></span
        >
        <input
          type="range"
          min="0"
          max="100"
          step="2"
          :value="settings.lyrics.desktopGradientOpacity * 100"
          @change="setNumber('desktopGradientOpacity', $event, 0, 1, 100)"
        />
      </label>
      <label class="range-item">
        <span
          >描边宽度 <b>{{ settings.lyrics.desktopStrokeWidth.toFixed(1) }} px</b></span
        >
        <input
          type="range"
          min="0"
          max="4"
          step="0.5"
          :value="settings.lyrics.desktopStrokeWidth"
          @change="setNumber('desktopStrokeWidth', $event, 0, 4)"
        />
      </label>
    </div>
    <div class="option-grid compact weight-options">
      ...
      />
      <BaseCheckbox
        id="setting_dl_shadow"
        :model-value="settings.lyrics.desktopShadowEnabled"
        label="显示文字阴影"
        @update:model-value="updateLyrics({ desktopShadowEnabled: $event as boolean })"
      />
      <BaseCheckbox
        id="setting_dl_gradient_bar"
        :model-value="settings.lyrics.desktopGradientBar"
        label="显示黑色渐变底条"
        @update:model-value="updateLyrics({ desktopGradientBar: $event as boolean })"
      />
    </div>
  </dd>
  ...
  <dd>
    ...
    <div class="color-grid">
      ...
      <label class="color-item">
        <input
          type="color"
          :value="settings.lyrics.desktopStrokeColor"
          @input="setColor('desktopStrokeColor', $event)"
        />
        <span>文字描边</span>
        <code>{{ settings.lyrics.desktopStrokeColor }}</code>
      </label>
    </div>

    <div class="lyric-preview" :style="previewStyle">
      <span class="preview-active">坤音neo 把旋律留在此刻</span>

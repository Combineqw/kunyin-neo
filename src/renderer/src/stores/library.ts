/**
 * 本地曲库 Pinia store。
 * 维护主进程 SQLite 数据的渲染层缓存，并以系统 favorites 歌单作为唯一收藏数据源。
 */
 ...
 */
/**
 * 提供本地歌单、收藏索引及曲库 IPC 操作入口。
 *
 * @returns 曲库响应式缓存与对应的读取、写入、广播订阅方法。
 */
export const useLibraryStore = defineStore('library', () => {
  ...
  async function toggleFavorite(item: MusicItem): Promise<boolean> {
    ...
    // 心动模式立即重排尚未播放的候选，收藏状态本身仍只有 favorites 系统歌单这一份数据源。
    void usePlayerStore().refreshHeartbeatAfterFavoriteChange()
    return now
  }

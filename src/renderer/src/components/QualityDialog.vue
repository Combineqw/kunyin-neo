...
<template>
  <!-- Teleport 到 body：调用方（如 .song-row）可能带 transform/filter，
       那会成为 position:fixed 的包含块，把整屏遮罩压缩进调用方盒子里。 -->
  <Teleport to="body">
    <div class="mask" @click.self="emit('close')">
      <div class="dialog">
        ...
        <div class="btns">
          <button v-for="o in options" :key="o.id" class="qbtn pressable" @click="pick(o.id)">
            {{ o.label }}
          </button>
        </div>
        <div class="foot">

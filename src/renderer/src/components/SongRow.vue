<!--
  通用歌曲列表行：统一承载播放、收藏、下载、MV 与歌单操作的展示入口。
  业务写入仍委托给 Pinia store 和主进程 IPC，组件只维护行内弹窗与菜单状态。
-->
...
<template>
  <div
    class="song-row pressable pressable-subtle"
  ...
  >
    <div class="cell cover">
      <div class="row-cover" :class="{ placeholder: !cover }">
        <img v-if="cover" :src="cover" loading="lazy" alt="" />
        <AppIcon v-else name="library" :size="20" />
        <span v-if="active" class="cover-play"><AppIcon name="play" :size="16" /></span>
      </div>
    </div>
    <div class="cell name">
      <span class="song-title ellipsis">{{ item.title }}</span>
      <span v-if="qualityInfo" class="quality-tag" :class="qualityInfo.tier">{{
        qualityInfo.label
      }}</span>
    ...
    <div class="cell ops">
      <template v-if="settingsStore.settings.list.showOperationButtons">
        <button
          class="op favorite pressable"
          :class="{ liked: library.isFavorite(item) }"
          :title="library.isFavorite(item) ? '取消收藏' : '收藏'
          "
          @click.stop="onFavorite"
        >
          <AppIcon :name="library.isFavorite(item) ? 'heart-filled' : 'heart'" :size="16" />
        </button>
        <button class="op pressable" title="试听" @click.stop="emit('play')">
          <AppIcon name="headphone" :size="16" />
        </button>
        <button class="op pressable" title="添加到列表" @click.stop="addDialog = true">
          <AppIcon name="add-to" :size="16" />
        </button>
        <button class="op pressable" title="下载" @click.stop="onDownload">

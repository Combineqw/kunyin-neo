/**
 * Preload 安全桥接入口。
 * 将公共 IPC 契约映射为最小化、类型化的 window.api；渲染层不直接持有 ipcRenderer 或 Node 能力。
 */
...
/**
 * 把响应式 Proxy 转为可通过 Electron structured clone 传输的普通对象。
 *
 * @param v 可能来自 Vue/Pinia 的调用参数。
 * @returns 可安全传入 ipcRenderer 的普通值。
 */
...
const api: WindowApi = {
  ...
  playlog: {
    record: (event) => ipcRenderer.invoke(IpcChannels.PLAYLOG_RECORD, toPlain(event))
  },
  recommendation: {
    songs: (limit) => ipcRenderer.invoke(IpcChannels.RECOMMENDATION_SONGS, limit),
    daily: (force) => ipcRenderer.invoke(IpcChannels.RECOMMENDATION_DAILY, force)
  },
  auth: {
  ...
  miniPlayer: {
    ...
    push: (state) => ipcRenderer.send(IpcChannels.MINI_PLAYER_PUSH, state),
    onState: (cb) => {
      const listener = (_e: Electron.IpcRendererEvent, state: MiniPlayerState): void => cb(state)
      ipcRenderer.on(IpcChannels.MINI_PLAYER_STATE, listener)
      return () => {
        ipcRenderer.off(IpcChannels.MINI_PLAYER_STATE, listener)
      }
    },
    command: (command: MiniPlayerCommand) =>
      ipcRenderer.send(IpcChannels.MINI_PLAYER_COMMAND, command),
    setExpanded: (expanded) => ipcRenderer.send(IpcChannels.MINI_PLAYER_SET_EXPANDED, expanded)
  },
  download: {
    add: (input) =>
  ...
  shell: {
    ...
    selectOpenFile: (filters) => ipcRenderer.invoke(IpcChannels.DIALOG_SELECT_OPEN_FILE, filters),
    selectSaveFile: (defaultName, filters) =>
      ipcRenderer.invoke(IpcChannels.DIALOG_SELECT_SAVE_FILE, defaultName, filters),
  ...
  animation: {
    ...
    exportPack: (pack) => ipcRenderer.invoke(IpcChannels.ANIMATION_EXPORT_FILE, toPlain(pack))
  },
  theme: {
  ...
  pluginStore: {
    list: () => ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_LIST),
    install: (id) => ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_INSTALL, id),
    uninstall: (id) => ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_UNINSTALL, id),
    setEnabled: (id, enabled) =>
      ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_SET_ENABLED, id, enabled),
    stageLocal: (filePath) => ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_STAGE_LOCAL, filePath),
    pickLocal: () => ipcRenderer.invoke(IpcChannels.PLUGIN_STORE_PICK_LOCAL),
    onChange: (cb) => {
      const listener = (_e: Electron.IpcRendererEvent, event: PluginStoreEvent): void => cb(event)
      ipcRenderer.on(IpcChannels.PLUGIN_STORE_CHANGED, listener)
      return () => ipcRenderer.off(IpcChannels.PLUGIN_STORE_CHANGED, listener)
    }
  },
  pluginRepo: {
    list: () => ipcRenderer.invoke(IpcChannels.PLUGIN_REPO_LIST),
    add: (url, name) => ipcRenderer.invoke(IpcChannels.PLUGIN_REPO_ADD, url, name),
    remove: (id) => ipcRenderer.invoke(IpcChannels.PLUGIN_REPO_REMOVE, id),
    setEnabled: (id, enabled) =>
      ipcRenderer.invoke(IpcChannels.PLUGIN_REPO_SET_ENABLED, id, enabled)
  },
  account: {

...
日期：2026-09-06；时间均为 UTC+08:00。本文为工单5终稿，只记录操作和观测，不作验收通过判定。
...
- 原两笔提交 b06947f、73d1151 已推送。本单提交前远端 HEAD 为 `fdabcc24699847a5ca1968502929a4fb5f5bd01f`，包含 README 重写。
...
| dev B（CDP 自动） | 13:54:41 | 30.534 秒 | 4 | **426.58 MiB** | **209.00 MiB** |
...
- 正式包快捷键受限，故其 A 保留为独立用户基线，B 按工单5使用 dev。历史 dev A 与本次 dev B 属于不同启动轮次，B 另启调试端口，**不计算 A−B，也不跨正式包/dev 相减**。
